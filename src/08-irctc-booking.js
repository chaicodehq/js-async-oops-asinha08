...
export async function checkSeatAvailability(trainNumber, date, classType) {
  await new Promise(r => setTimeout(r, 100)); // Simulates API call delay
  ...
  const seats = Math.floor(Math.random() * 51); // 0 to 50
  const waitlist = Math.floor(Math.random() * 21); // 0 to 20
...
export async function raceBooking(trainNumbers, passenger, date, classType) {
  try {
    const promises = trainNumbers.map(tn => bookTicket(passenger, tn, date, classType).then(res => {
      if (res.status === "waitlisted") throw new Error("Waitlisted");
      return res;
    }));
