...
export class Incubator {
  ...
  async admitStartup(startup) {
    ...
    if (this.#startups.some(wrapper => wrapper.startup.name === startup.name)) {
  ...
  removeStartup(name) {
    const idx = this.#startups.findIndex(w => w.startup.name === name);
  ...
  async assignMentor(startupName, mentor) {
    await new Promise(r => setTimeout(r, 50));
    const wrapper = this.#startups.find(w => w.startup.name === startupName);
    if (!wrapper) throw new Error("Startup not found!");
  ...
  getStartupsByDomain(domain) {
    return this.#startups
      .filter(w => w.startup.domain === domain)
      .map(w => w.startup);
  }
